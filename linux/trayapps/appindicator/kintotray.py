...
class Indicator:
    ...
    if sysv:
        kinto_status = Popen(
            "export TERM=xterm-color;while :; do clear; pgrep 'xkeysnail' && echo 'active'; sleep 2; done",
            stdout=PIPE,
            shell=True,
        )
    else:
    ...
    def __init__(self):
        ...
        if res.returncode == 0:
            ...
            self.indicator = appindicator.Indicator.new(
                APPINDICATOR_ID,
                os.environ["HOME"] + "/.config/kinto/kinto-invert.svg",
            ...
            self.indicator = appindicator.Indicator.new(
                APPINDICATOR_ID,
                os.environ["HOME"] + "/.config/kinto/kinto.svg",
                appindicator.IndicatorCategory.SYSTEM_SERVICES,
            )
    ...
    def build_menu(self, res):
        ...
        if autostart_bool:
            ...
            self.chkautostart_id = self.checkbox_autostart.connect(
        ...
        if os.path.exists(os.environ["HOME"] + "/.config/autostart/kintotray.desktop"):
            ...
            self.systray.signal_id = self.systray.connect(
    ...
    def non_block_read(self):
        ...
        if (
            "inactive" in status
            or "failed" in status
            or "deactivating" in status
            or "activating" in status
        ):
            stats = "inactive"
    ...
    def update_terminal(self):
        ...
        nowts = int(time.time())
        if (nowts - self.unixts) > 5 and (
            status == "active"
            and self.indicator.get_icon()
            != os.environ["HOME"] + "/.config/kinto/kinto-invert.svg"
        ):
            self.indicator.set_icon(
                os.environ["HOME"] + "/.config/kinto/kinto-invert.svg"
            )
        elif (nowts - self.unixts) > 5 and (
            status == "inactive"
            and self.indicator.get_icon()
            != os.environ["HOME"] + "/.config/kinto/kinto.svg"
        ):
    ...
    def setTweaks(self, button):
        ...
        self.caps2esc = Gtk.CheckButton(
            label="Capslock is Escape when tapped, Cmd when held"
        )
        ...
        try:
            if opendebug:
                Popen(
                    [
                        "/usr/bin/python3",
                        os.environ["HOME"] + "/.config/kinto/gui/kinto-gui.py",
                        "-d",
                    ]
                )
            else:
