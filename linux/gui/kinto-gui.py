...
from gi.repository import Gdk, GdkPixbuf, GLib, Gtk, Vte
...
class MyWindow(Gtk.Window):
    ...
    try:
        sysv = int(
            Popen(
                "pidof systemd >/dev/null 2>&1 && echo '0' || echo '1'",
                stdout=PIPE,
                shell=True,
            )
            .communicate()[0]
    ...
    if sysv:
        kinto_status = Popen(
            "while :; do clear; pgrep 'xkeysnail' && echo 'active'; sleep 2; done",
            stdout=PIPE,
            shell=True,
        )
    else:
        kinto_status = Popen(
            "while :; do clear; systemctl is-active xkeysnail; sleep 2; done",
            stdout=PIPE,
            shell=True,
        )
    ...
    def __init__(self):
        ...
        parser.add_argument(
            "-d", dest="debug", action="store_true", help="runs kinto in debug mode"
        )
        parser.add_argument(
            "--debug",
            dest="debug",
            action="store_true",
            help="runs kinto in debug mode",
        )
        parser.add_argument(
            "-s", dest="setup", action="store_true", help="first time setup"
        )
        parser.add_argument(
            "--setup", dest="setup", action="store_true", help="first time setup"
        )
        ...
        if autostart_bool:
            ...
            self.menuitem_auto.signal_id = self.menuitem_auto.connect(
                "activate", self.setAutostart, False
            )
        else:
            self.menuitem_auto.set_active(False)
            self.menuitem_auto.signal_id = self.menuitem_auto.connect(
                "activate", self.setAutostart, True
            )
        ...
        kintotray = int(
            self.queryConfig(
                'ps -aux | grep [k]intotray >/dev/null 2>&1 && echo "1" || echo "0"'
            )
        )
        if kintotray and os.path.exists(
            os.environ["HOME"] + "/.config/autostart/kintotray.desktop"
        ):
            self.menuitem_systray.set_active(True)
            self.menuitem_systray.signal_id = self.menuitem_systray.connect(
                "activate", self.checkTray, False
            )
        else:
            self.menuitem_systray.set_active(False)
            self.menuitem_systray.signal_id = self.menuitem_systray.connect(
                "activate", self.checkTray, True
            )
        ...
        self.chromekb.signal_id = self.chromekb.connect(
            "activate", self.setKB, "chrome"
        )
        self.ibmkb.signal_id = self.ibmkb.connect("activate", self.setKB, "ibm")
        self.winmackb.signal_id = self.winmackb.connect(
            "activate", self.setKB, "winmac"
        )
    ...
    def setKinto(self):
        ...
        if self.options["kbtype"] == "mac":
            ...
            if os.path.isfile("/sys/module/hid_apple/parameters/swap_opt_cmd"):
                with open(
                    "/sys/module/hid_apple/parameters/swap_opt_cmd", "r"
                ) as ocval:
                    optcmd = ocval.read().replace("\n", "")
                if optcmd == "1":
                    # print("found hid_apple - remove")
                    self.queryConfig(
                        "echo '0' | sudo tee /sys/module/hid_apple/parameters/swap_opt_cmd;echo 'options hid_apple swap_opt_cmd=0' | sudo tee /etc/modprobe.d/hid_apple.conf;sudo update-initramfs -u -k all"
                    )
            if os.path.isfile("/sys/module/applespi/parameters/swap_opt_cmd"):
                ...
                if optcmd == "1":
                    ...
                    self.queryConfig(
                        "echo '0' | sudo tee /sys/module/applespi/parameters/swap_opt_cmd;echo 'options applespi swap_opt_cmd=0' | sudo tee /etc/modprobe.d/applespi.conf;sudo update-initramfs -u -k all"
                    )
            ...
            self.queryConfig(
                'perl -pi -e "s/(# )(.*)(# xfce4)/\$2\$3/g" ~/.config/kinto/kinto.py'
            )
            self.queryConfig(
                'perl -pi -e "s/(\w.*)(# Default)/# \$1\$2/g" ~/.config/kinto/kinto.py'
            )
        ...
        if self.options["autostart"]:
            self.queryConfig(
                "cp "
                + os.environ["HOME"]
                + "/.config/kinto/xkeysnail.desktop "
                + os.environ["HOME"]
        ...
        if self.options["systray"]:
            self.queryConfig(
                "cp "
                + os.environ["HOME"]
                + "/.config/kinto/kintotray.desktop "
                + os.environ["HOME"]
                + "/.config/autostart/kintotray.desktop"
            )
            self.queryConfig(
                "nohup /usr/bin/python3 ~/.config/kinto/kintotray.py >/dev/null 2>&1 &"
            )
    ...
    def initSetup(self):
        ...
        if checkkb_result == 0:
            ...
            tokenValue = re.findall(
                r"(e|d|3|k|y|t)\s=\s(\w+)", initkb_file, re.MULTILINE
            )
        ...
        if self.options["kbtype"] != "ask":
            ...
            img2 = Image.open(
                os.environ["HOME"] + "/.config/kinto/gui/capslock_1200x720.png"
            )
            img3 = Image.open(
                os.environ["HOME"] + "/.config/kinto/gui/keys_1200x720.png"
            )
            ...
            pixbuf = GdkPixbuf.Pixbuf.new_from_file(
                os.environ["HOME"] + "/.config/kinto/gui/tuxcry4.png"
            )
            ...
            pixbuf = GdkPixbuf.Pixbuf.new_from_file(
                os.environ["HOME"] + "/.config/kinto/gui/tuxuninstall.png"
            )
            pixbuf = pixbuf.scale_simple(600, 360, GdkPixbuf.InterpType.BILINEAR)
            ...
            pixbuf = GdkPixbuf.Pixbuf.new_from_file(
                os.environ["HOME"] + "/.config/kinto/gui/tuxbg.png"
            )
            pixbuf = pixbuf.scale_simple(600, 360, GdkPixbuf.InterpType.BILINEAR)
    ...
    def refreshFile(self, button):
        ...
        if (
            os.path.exists(os.environ["HOME"] + "/.config/autostart/kintotray.desktop")
            and kintotray
            and self.menuitem_systray.get_active() == False
        ):
            self.menuitem_systray.disconnect(self.menuitem_systray.signal_id)
            self.menuitem_systray.set_active(True)
            self.menuitem_systray.signal_id = self.menuitem_systray.connect(
            ...
            os.path.exists(os.environ["HOME"] + "/.config/autostart/kintotray.desktop")
            and not kintotray
            and self.menuitem_systray.get_active() == True
        ):
            self.menuitem_systray.disconnect(self.menuitem_systray.signal_id)
            self.menuitem_systray.set_active(False)
            self.menuitem_systray.signal_id = self.menuitem_systray.connect(
    ...
    def checkTray(self, button, tray_bool):
        ...
        if tray_bool:
            Popen(
                [
                    "cp",
                    os.environ["HOME"] + "/.config/kinto/kintotray.desktop",
                    os.environ["HOME"] + "/.config/autostart/kintotray.desktop",
                ]
            )
            if not kintotray:
                Popen(
                    [
                        "/usr/bin/python3",
                        os.environ["HOME"] + "/.config/kinto/kintotray.py",
                    ]
                )
            self.menuitem_systray.disconnect(self.menuitem_systray.signal_id)
            self.menuitem_systray.set_active(True)
            ...
            global child_pid
            if sysv:
                self.kinto_status = Popen(
                    "export TERM=xterm-color;while :; do clear; pgrep 'xkeysnail'; sleep 2; done",
                    stdout=PIPE,
                    shell=True,
                )
            else:
                self.kinto_status = Popen(
                    "export TERM=xterm-color;while :; do clear; systemctl is-active xkeysnail; sleep 2; done",
                    stdout=PIPE,
                    shell=True,
                )
    ...
    def setKB(self, button, kbtype):
        ...
        try:
            if kbtype == "win":
                ...
                if os.path.isfile("/sys/module/hid_apple/parameters/swap_opt_cmd"):
                    with open(
                        "/sys/module/applespi/parameters/swap_opt_cmd", "r"
                    ) as ocval:
                        optcmd = ocval.read().replace("\n", "")
                    if optcmd == "0":
                        # print("found hid_apple")
        ...
        if restartsvc == True:
            try:
                ...
                Popen(
                    [
                        "notify-send",
                        "Kinto: Error restarting Kinto after setting tweaks!",
                    ]
                )
    ...
    def setVSC2ST3(self, button):
        ...
        try:
            ...
            Popen(
                ["notify-send", "Kinto: Error Resetting SublimeText remaps for VSCode!"]
            )
        return
    ...
    def setCaps2Esc(self, button):
        ...
        try:
            if (
                self.winkb.get_active()
                or self.winmackb.get_active()
                or self.ibmkb.get_active()
                or self.mackb.get_active()
            ):
            ...
            if autostart == False:
                Popen(
                    [
                        ...
                        "-pi",
                        "-e",
                        "s/autostart = true/autostart = false/g",
                        os.environ["HOME"] + "/.config/kinto/kinto.py",
                    ]
                )
                self.menuitem_auto.set_active(False)
    ...
    def setConfig(self, button):
        try:
            ...
            CalledProcessError
        ):  # Notify user about error on running restart commands.
    ...
    def key_press_event(self, widget, event):
        ...
        if self.page == 1 and "Control" in keyname and openWin == False:
            ...
            self.page == 2
            and "Caps_Lock" in keyname
            and event.state & Gdk.ModifierType.LOCK_MASK
        ):
...
class FirstPage(Gtk.Box):
    ...
    def forward(self, button):
        ...
        self.__parent_window.setupwin.signal_id = self.__parent_window.setupwin.connect(
            "key_press_event", self.__parent_window.key_press_event
        )
        # print(self.__parent_window.setupwin.signal_id)
...
class SecondPage(Gtk.Box):
    ...
    def goback(self, *args):
        ...
        self.__parent_window.setupwin.disconnect(
